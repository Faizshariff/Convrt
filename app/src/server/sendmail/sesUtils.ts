...
// const mergeTagBatches = chunkArray(mergeTags, 1000); // Corresponding merge tag batches

// Process each batch with concurrency control
const batchPromises = batches.map((batch, batchIndex) => {
  return queue.add(async () => {
    const personalizations = batch.map((email: any, index: any) => {
      return {
        to: email,
        custom_args: { unique_arg: idBatches[batchIndex][index] },
     //   dynamic_template_data: mergeTagBatches[batchIndex][index].tags,
      };
    });
    ...
    try {
      console.log("///////////////// this is message strcuture ///////////////",msg.personalizations)
